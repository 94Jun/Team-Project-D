...
const MyPagePost = () => {
  ...
  const getPostingList = async () => {
    if (!lastVisible) {
      const first = query(collection(db, "postingList"), where("pid", "in", testUser?.markedPosting), orderBy("timestamp", "desc"), limit(5));
      const querySnapshot = await getDocs(first);
      setLastVisible(querySnapshot.docs[querySnapshot?.docs?.length - 1]);
      const loadedData = querySnapshot.docs.map((doc) => doc.data());
      setPostingList(loadedData);
    } else { 
      const next = query(collection(db, "postingList"), where("pid", "in", testUser?.markedPosting), orderBy("timestamp", "desc"), startAfter(lastVisible), limit(5));
